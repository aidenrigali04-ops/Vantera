...
- [ ] TDD: alpha.ts pure tests (spend curve, pause at exhaustion, cap); optimize.test.ts updates (V2 wired — rig flags so V2 adopts: e.g. 6/40 vs 16/40 gives e≥20, verify with the math module first; chain pauses at exhausted wealth; legacy null alphaSpent uses default). Jobs suite + type-check green.
