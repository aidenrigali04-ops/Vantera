...
/**
 * LLM judge for cold-outreach copy quality (Phase 2B, Task 6).
 *
 * ADVISORY ONLY — nothing in the pipeline gates on this judge until `runCalibration` (see
 * `./kappa`) reports `trusted: true`, i.e. the judge's binary good/bad calls agree with a real
 * human rater at Cohen's kappa >= 0.7 on a hand-labeled sample
 ...
 * only: no call site may fail a draft, block a send, or feed this score into another gate.
 *
...
// attributable to an exact prompt revision (enterprise-grade-brain WS-2.1 convention).
