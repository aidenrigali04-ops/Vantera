...
export const linkedinAccounts = pgTable(
  'linkedin_accounts',
  {
    ...
    unipileConnected: boolean('unipile_connected').notNull().default(false),
    unipileConnectedAt: timestamptz('unipile_connected_at'),
    unipileLastError: text('unipile_last_error'),
    createdAt: timestamptz('created_at').notNull().defaultNow(),
    updatedAt: timestamptz('updated_at').notNull().defaultNow(),
  },
  (table) => ({
