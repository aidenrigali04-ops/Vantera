...
const TEMPLATES: Record<LifecycleSegment, { touch1: Builder[]; touch2: Builder[] }> = {
  ...
  idle_after_onboarding: {
    touch1: [
      (d) =>
        d.leadCount > 0
          ? `${greet(d.firstName)} your scout agent found ${d.leadCount} leads since you set it up${d.qualifiedCount > 0 ? ` and ${d.qualifiedCount} passed your qualification bar` : ""}. They're in your dashboard when you have a minute.`
  ...
  trial_lapsed: {
    touch1: [
      (d) =>
        d.leadCount > 0
          ? `${greet(d.firstName)} your Vantera trial wrapped up. While it ran, your scout found ${d.leadCount} leads${d.qualifiedCount > 0 ? ` and ${d.qualifiedCount} qualified` : ""}. They're going cold sitting there. Want me to turn it back on for you?`
      ...
      (d) =>
        d.leadCount > 0
          ? `${greet(d.firstName)} your trial ended with ${d.leadCount} leads found${d.qualifiedCount > 0 ? ` and ${d.qualifiedCount} qualified` : ""}. That pipeline is just parked now. Happy to restart it if you want to keep going.`
