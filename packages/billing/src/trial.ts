 ...
 * migration 0019). The first-deploy gate passes because `subscriptionStatus`
 * is 'trialing' (isActive) on a real tier — see entitlements.isActive.
 */
export const TRIAL_TIER: PlanTier = "starter";

/** Trial length in days. Must match the `now() + interval '14 days'` default in 0019. */
