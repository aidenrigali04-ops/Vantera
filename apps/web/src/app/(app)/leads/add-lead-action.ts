...
export async function addManualLead(formData: FormData): Promise<AddLeadState> {
  ...
  const { data: suppressed } = await supabase
    ...
    .maybeSingle();
  if (suppressed) {
    return { error: "That profile is on your do-not-contact list — it can't be added as a lead." };
  }
  ...
  const { data: existing } = await supabase
    ...
    .limit(1)
    .maybeSingle();
  if (existing) return { error: "Already in your leads.", leadId: existing.id };
  ...
  const { data: inserted, error } = await supabase
    ...
    .maybeSingle();
  if (error || !inserted) {
    return { error: "Could not add the lead. Only workspace admins can add leads." };
  }
