...
export const metadata = { title: "Senders" };
...
export default async function ChannelsPage({
  ...
  if (connected === "1" && accountRow?.id) {
    try {
      ...
      console.error("senders reconcile on connect failed:", err);
    }
  }
          ...
          {linkedinAccounts.length === 0 ? (
            ...
            <div className="space-y-4">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {/* id anchor on each row: Today's repair tiles + Engine line deep-link to
                      /settings/senders#{id}, so a repair lands on the sender that needs it. */}
                  {linkedinAccounts.map((la) => (
                    <tr key={la.id} id={la.id} className="scroll-mt-24 border-t border-border target:bg-[var(--attention-tint)]">
