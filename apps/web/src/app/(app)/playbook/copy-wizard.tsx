...
export function CopyWizard({
  ...
  return (
    <WizardShell
      ...
      hint={
        [
          "Your outreach teammate — it writes every LinkedIn message, tailored to each prospect.",
          ...
          <div className="flex flex-col gap-4">
            ...
            {linkedinCount === 0 && (
              <p className="rounded-md border border-amber-500/30 bg-amber-500/5 px-3 py-2 text-xs text-muted-foreground">
                ...
                <a href="/settings/senders" className="underline underline-offset-2">
                  connect LinkedIn in Settings → Senders
                </a>
                .
              </p>
            )}
            ...
            <p className="rounded-lg border border-primary/30 bg-primary/5 px-4 py-3 text-sm">
              {name.trim() || "Your agent"} starts drafting as soon as qualified prospects arrive.{" "}
              {sendMode === "review"
                ? "Every draft waits in Approvals — nothing sends without you."
                : "Clean drafts send automatically on a human-like schedule; flagged ones wait in Approvals."}
            </p>
          </div>
        )}
