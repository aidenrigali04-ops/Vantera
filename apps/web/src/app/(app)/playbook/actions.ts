...
export async function deployScoutAgent(
  ...
  if (agentError || !agent) {
    if (agentError?.code === "23505") {
      return { error: "You already have a Prospect Agent. Pause or edit it from the Playbook page." };
    }
  ...
  redirect("/playbook?deployed=scout");
}
...
export async function deployIntentAgent(
  ...
  if (agentError || !agent) {
    if (agentError?.code === "23505") {
      return { error: "You already have an Intent Agent. Pause or edit it from the Playbook page." };
    }
    return { error: "Could not deploy the agent. Only workspace admins can do this." };
  }

  revalidatePath("/playbook");
  redirect("/playbook?deployed=intent");
}
...
export async function deployCopyAgent(
  ...
  if (!parsed.ok) return { error: parsed.error };
  const { name, cta, bookingUrl, websiteUrl, links, channels, sendMode } = parsed.values;
  ...
  const { data: agent, error: agentError } = await supabase
    ...
    .insert({
      ...
      name,
      status: "live",
      config: { cta, bookingUrl, websiteUrl, channels },
  ...
  if (agentError || !agent) {
    if (agentError?.code === "23505") {
      return { error: "You already have an Outreach Agent. Pause or edit it from the Playbook page." };
    }
    return { error: "Could not deploy the agent. Only workspace admins can do this." };
  }
  ...
  revalidatePath("/playbook");
  redirect("/playbook?deployed=copy");
}
...
export async function updateScoutAgent(
  ...
  revalidatePath("/playbook");
  redirect("/playbook?updated=scout");
}
...
export async function updateCopyAgent(
  ...
  const { error: updateError } = await supabase
    .from("agents")
    .update({ name, config: { ...(agent.config ?? {}), cta, bookingUrl, websiteUrl, channels } })
    .eq("id", agent.id); // RLS scopes to the admin's account (rule 02)
  if (updateError) return { error: "Could not save changes. Only workspace admins can do this." };
  ...
  revalidatePath("/playbook");
  redirect("/playbook?updated=copy");
}
