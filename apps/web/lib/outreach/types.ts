...
export function parseCampaignWorkflow(raw: unknown): OutreachCampaignWorkflow {
  ...
  const automation =
    workflow.automation &&
    workflow.automation.source === 'prospect_scout' &&
    typeof workflow.automation.aspireSearchRunId === 'string'
      ? workflow.automation
      : undefined
  ...
  return { steps, deliveryMode, channelFocus, automation }
}

export function isAutoScoutCampaignWorkflow(workflow: OutreachCampaignWorkflow): boolean {
  return Boolean(workflow.automation?.autoGenerated && workflow.automation.source === 'prospect_scout')
}
