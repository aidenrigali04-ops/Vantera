...
function HubRelatedLink({ link }: { link: AdminNavHubLink }) {
  const content = (
    <Link
      ...
      className="inline-flex items-center rounded-md px-2 py-1 text-[12px] font-medium text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-overlay)] hover:text-[var(--text-primary)]"
    >
      {link.label}
    </Link>
  )
...
function HubCard({ hub }: { hub: AdminNavHub }) {
  ...
  return (
    <article className="flex h-full flex-col rounded-xl border border-[var(--border-default)] bg-white p-4 shadow-sm">
      <div className="flex items-start gap-3">
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[var(--bg-overlay)] text-[var(--text-secondary)]">
          <PrimaryIcon className="h-4 w-4" aria-hidden />
        </span>
        <div className="min-w-0 flex-1">
          <h3 className="text-[14px] font-semibold text-[var(--text-primary)]">{hub.title}</h3>
          <p className="mt-0.5 text-[12px] leading-relaxed text-[var(--text-tertiary)]">{hub.description}</p>
        </div>
      </div>
      ...
      {!inSidebar ? (
        <Link
          ...
          className="mt-4 inline-flex h-9 items-center justify-center gap-1.5 rounded-lg bg-[var(--accent)] px-3 text-[13px] font-medium text-[var(--text-inverse)] transition-colors hover:bg-[var(--accent-hover)]"
        >
          {hub.primary.label}
      ...
      {hub.related.length > 0 ? (
        <div className={cn('mt-3 flex flex-wrap gap-1', !inSidebar && 'mt-4 border-t border-[var(--border-subtle)] pt-3')}>
  ...
  return (
    <section className={cn('space-y-3', className)} aria-label="Workspace areas">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h2 className="text-[15px] font-semibold tracking-[-0.01em] text-[var(--text-primary)]">Where to go next</h2>
