...
One primary discovery provider (Apollo-class contact DB via API) as source of truth, plus a Vantera-orchestrated **enrichment waterfall**: cheap checks first; premium enrichment (e.g. Clay) spent only on leads that pass the scoring gate. The industry/ICP-tailoring logic lives in Vantera's enrichment orchestrator, not in any one provider.
